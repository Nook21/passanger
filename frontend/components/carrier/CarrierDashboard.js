...
export default function CarrierDashboard({ route }) {
  ...
  // ✅ Load user and token from AsyncStorage (consistent with SignIn & SenderDashboard)
  useEffect(() => {
  ...
  const fetchFlights = async () => {
    try {
      const res = await fetch(`${BASE_URL}/api/flights/get`);
  ...
  const fetchShipments = async () => {
    try {
      const res = await fetch(`${BASE_URL}/api/flights/shipments`, {
        headers: {
  ...
  const handleAddFlight = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/flights/add`, {
        method: 'POST',
        headers: {
  ...
  const renderFlight = ({ item }) => (
    <View style={styles.flightCard}>
      <Text style={styles.flightTextBold}>
        {item.from} - {item.to}
      </Text>
      <View style={styles.flightDetailsRow}>
        <Text style={styles.flightTextDetail}>
        ...
        <Text
          style={[
            styles.flightTextStatus,
            { color: item.status === 'Active' ? '#4CAF50' : '#FFB733' },
          ]}
        >
          {item.status}
        </Text>
        ...
        <View style={styles.sidebar}>
          <View style={styles.profileContainer}>
            ...
            <Text style={styles.profileName}>
              {user ? user.fullName || 'Loading...' : 'Loading...'}
            </Text>
              ...
              renderItem={({ item }) => (
                <View style={styles.shipmentCard}>
                  <Text style={styles.shipmentTitle}>
                    {item.sender.fullName} → {item.flight.from} to{' '}
                    {item.flight.to}
                  </Text>
                  <Text style={styles.shipmentDetail}>
                    Weight: {item.itemWeight} Kg | Status:{' '}
                    {item.acceptorVerified ? 'Verified' : 'Pending'}
                  </Text>
                  <Text style={styles.shipmentDetail}>
                  ...
                  <Text style={styles.shipmentDetail}>
                    Departure:{' '}
                    {new Date(item.flight.departureDate).toLocaleString()}
                  </Text>
                </View>
              )}
              scrollEnabled={false}
...
const styles = StyleSheet.create({
  ...
  profileEmail: { color: 'rgba(255, 255, 255, 0.6)', fontSize: 11 },
  sidebarLink: { paddingVertical: 10, paddingHorizontal: 5, borderRadius: 8 },
  activeSidebarLink: {
  ...
  sidebarText: { color: '#fff', fontWeight: '600', fontSize: 13 },
  activeSidebarText: { color: '#2D4B46', fontWeight: 'bold' },
  activeUsersContainer: { marginBottom: 20, alignItems: 'center' },
  activeUsersTitle: {
  ...
  content: { flexGrow: 1, padding: contentPadding },
  card: {
    backgroundColor: '#fff',
    borderRadius: 15,
    padding: 20,
  ...
  addText: { color: '#2D4B46', fontWeight: 'bold', textAlign: 'center' },
  subtitle: {
    color: '#2D4B46',
    fontSize: 18,
    fontWeight: 'bold',
  ...
  flightTextDetail: { color: '#888', fontSize: 12, marginRight: 10 },
  flightTextStatus: { fontSize: 12, fontWeight: 'bold' },
  shipmentCard: {
    backgroundColor: '#fff',
    borderRadius: 10,
    padding: 15,
  ...
  shipmentDetail: { color: '#888', fontSize: 12, marginBottom: 2 },
});
